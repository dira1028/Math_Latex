...
\subsection{Linearity of Expectation (Additivity)}
...
(Note: The converse($Cov=0\rightarrow$ independent) is generally not true. )\hfill [536,537]
...
\subsection{Convolution of Discrete Case}
Assume that you have a pair $(X,Y)$ of independent discrete random variables $X$ and $Y$, with PMFs $f(x)$ and $g(y)$ respectively. Their joint probability mass function is given by:
$$p(x,y) = P(X=x, Y=y) = P(X=x)P(Y=y) = f(x)g(y)$$
Using the indicator function $I_A$:
\begin{align*}
    I_A(x,y)      & = \begin{cases} 1 & \text{if } (x,y) \in A \\ 0 & \text{if } (x,y) \notin A \end{cases} \\
    P((X,Y)\in A) & = \sum_{(x,y)\in A}p(x,y) = \sum_x\sum_y p(x,y) \cdot I_{A}(x,y)
\end{align*}
We want to know the distribution of $Z=X+Y$.\\
Let $A=\{(x,y)\mid x+y=z\}$. Then:
\begin{align*}
    P(Z=z) & = P((X,Y)\in A) = \sum_{y} p(z-y, y) \\
           & = \sum_{y} P(X=z-y)P(Y=y)            \\
           & = \sum_{y} f(z-y)g(y)
\end{align*}

\subsection{Convolution on Finite Groups}
Assume that the independent random variables $X$ and $Y$ take values on a finite group $G$ with group operation $\circ$. Let $X$ and $Y$ be distributed according to $\tau$ and $\eta$ respectively.
\begin{align*}
    p(x,y) = P(X=x, Y=y) = P(X=x)P(Y=y) = \eta(x)\tau(y)
\end{align*}
Let $Z = Y \circ X$ and $A = \{(x,y) \in G \times G \mid y \circ x = z\}$. Since $y \circ x = z \iff y = z\circ x^{-1} $, we have:
\begin{align*}
    P(Z=z)=P((X,Y)\in A) & = \sum_{(x,y)\in A} p(x,y)                    \\
                         & = \sum_{x\in G} p(x, z \circ x^{-1})          \\
                         & = \sum_{x \in G} P(Y=z^{-1}\circ x) P(X=x)    \\
                         & = \sum_{x \in G} \eta(z \circ x^{-1}) \tau(x)
\end{align*}
...
\subsection{Gamma Distribution : Expectation and Variance}
\begin{itemize}
    \item $E(X)$
          \begin{align*}
...
\subsection{Markov inequality and Chebyshev inequality}
\begin{itemize}
    \item Markov's Inequality
          \begin{align*}
              P(X \ge a) \le \frac{E[X]}{a} \quad (\text{for } X \ge 0, a > 0)
          \end{align*}
          \begin{proof}
              Let indicator function $I_{\{X \ge a\}}$ be defined as:
              \begin{align*}
                  I = \begin{cases}
                          1 & \text{if } X \ge a \\
                          0 & \text{if } X < a
                      \end{cases}
              \end{align*}
              Since $X \ge 0$ and $a > 0$, the following inequality holds:
              \begin{align*}
                  I_{\{X \ge a\}}                 & \le \frac{X}{a} \quad (\text{since } X \ge 0) \\
                  E\left[ I_{\{X \ge a\}} \right] & \le E\left( \frac{1}{a}X \right)              \\
    ...
    \item{Chebyshev inequality}
          \begin{align*}
              P(\mid {X-\mu}\mid\leq k)\geq \frac{\sigma^2}{k^2}
          \end{align*}
          \begin{proof}
              by Markov indequality:
              \begin{align*}
                  P(\mid {X-\mu}\mid\leq k)=P((X-\mu)^2\leq k^2)\geq \frac{1}{k^2}E(X-\mu)^2=\frac{1}{k^2}Var(X)
              \end{align*}
          \end{proof}
\end{itemize}
...
\subsection{weak LLN}
Let $X_1,X_2\cdots$ be iid random variables with a finite first moment, $EX_i=\mu$ then:
$$\frac{X_1+X_2+\cdots +X_n}{n}\rightarrow\mu$$
as $n\rightarrow\infty$
\begin{proof}
    Denote $\mu = E[X], \sigma^2 = \text{Var}(X)$, and let $S_n = X_1 + \cdots + X_n$. Then:
    \begin{align*}
        E[S_n] & = E[X_1] + \cdots + E[X_n] = n\mu \quad \text{and} \quad \text{Var}(S_n) = n\sigma^2 \\
        \intertext{Therefore, by Chebyshev's inequality:}
        P\left( \left| \frac{S_n}{n} - \mu \right| > \epsilon \right)
               & = P\left( | S_n - n\mu | > n\epsilon \right)                                         \\
               & \le \frac{n\sigma^2}{(n\epsilon)^2}                                                  \\
               & = \frac{\sigma^2}{n\epsilon^2} \to 0 \quad \text{as } n \to \infty
    \end{align*}
\end{proof}


\href{https://grokipedia.com/page/Product_measure}{[534]}